...
describe("DAI Value check", function () {
  it("DAI Value should be > 0 ", async function () {
    ...
    let Dai = new ethers.Contract("0x6B175474E89094C44Da98b954EedeAC495271d0F", abi)

    let txn = await recepient.getPaid({value: ethers.utils.parseEther("1")})

    let bal = await Dai.balanceOf(owner.address)

    assert(ethers.utils.formatEther(bal), 0, "Balance is > 0")
